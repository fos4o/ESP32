...
struct Data
{
	...
	uint32_t m_WIFIProcessingTime = 0;
...
void setup()
{
	...
	if (BMPFound)
	{
		tempBMP = m_BMP280.readTemperature();
		pressBMP = m_BMP280.readPressure();
	}
	...
	dataStore.m_Data.m_PressBMPAvg += pressBMP;
...
void PrintDataOnDisplay()
{
	...
	sprintf(buf, "Battery:%13.2f", batV);
	display.print(buf);
	display.println();
...
void print_wakeup_reason()
{
	...
	{
		case 
			ESP_SLEEP_WAKEUP_EXT0: Serial.println("Wakeup caused by external signal using RTC_IO"); 
		break;
		
		case 
			ESP_SLEEP_WAKEUP_EXT1: Serial.println("Wakeup caused by external signal using RTC_CNTL"); 
		break;
